...
Obj::Obj(const vector<double> &vertices, const vector<int> &faces) : m_vertices(vertices), m_faces(faces) {}
...
Obj Obj::loadObj(string name) {
    ...
    while(!file.eof()){
        ...
        if(std::regex_match(line, matches, vertex)){
            ...
            int s = stoi(matches[2].str());
            int t = stoi(matches[3].str());
            faces.push_back(f);
            faces.push_back(s);
            faces.push_back(t);
        }
